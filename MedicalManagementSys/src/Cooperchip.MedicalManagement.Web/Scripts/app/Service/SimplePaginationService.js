(function () {
    ...
    app.service('SimplePaginate', function () {
        ...
        var pagesTotal = function () { return Math.ceil(itemsTotal() / perPage); }
        //vai para uma página específica 
        var goToPage = function (indice) {
            current = indice;
            if (indice > 0) { indice = indice * perPage; } var final = indice + perPage;
            if (final > itemsTotal()) { final = itemsTotal(); }
            return { data: data.slice(indice, final), from: indice + 1, to: final };
        }
        //avança paginação se possível 
        var next = function () { if (current + 1 == pagesTotal()) { return goToPage(current); } current++; return goToPage(current); }
        //volta paginação se possível 
        var prev = function () {
            if (current == 0) { return goToPage(current); } current--; return goToPage(current);
        }
